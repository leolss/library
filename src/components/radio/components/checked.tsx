 ...
 * @LastEditTime: 2021-09-15 14:10:33
 * @Description:
 */
import React, { memo, useMemo } from 'react';
import { RadioProps } from '../interface';

const Checked: React.FC<RadioProps> = memo((props) => {
  const {
    width = '26',
    height = '26',
    disabled,
  ...
  const bgColor = useMemo(() => {
    return disabled ? '#ccc' : backgroundColor;
  }, [disabled, backgroundColor]);

  const c = useMemo(() => {
    return disabled ? '#fff' : color;
  }, [disabled, color]);
              ...
              >
                <g id="椭圆形">
                  <use
                    ...
                    xlinkHref="#path-1"
                  ></use>
                  <use fill={bgColor} xlinkHref="#path-1"></use>
                </g>
                <path
